...
#define ISMAZE	0000004		/* room is a maze */
...
extern bool	After, Again, Allscore, Amulet, Door_stop, Fight_flush,
		...
		Lower_msg, Move_on, Msg_esc, Noscore,
		Passgo, Playing, Q_comm, Running, Save_msg, See_floor,
		Seenstairs, Stat_msg, Terse, To_death, Tombstone;

extern bool	Pack_used[];

extern char	Dir_ch, File_name[], Home[], Huh[], *Inv_t_name[],
		L_last_comm, L_last_dir, Last_comm, Last_dir, Move_dir,
		*Numname,
		Outbuf[], *P_colors[], *Progname, *R_stones[],
		*Release, Runch,
		*S_names[], Take, *Tr_name[], *Tr_mystery[], *Ws_made[], *Ws_type[];
...
char	*charge_str(), *choose_str(), *inv_name(), *killname(),
	*nullstr(), *num(), *pick_color(), *ring_num(),
	*set_mname(), *type_name(), *vowelstr();

int	_putchar(), close(), exit(), printf(), printw();

bool	dropcheck(), find_floor(), pack_room();

int	pack_char(), floor_ch();

coord	*find_dest(), *rndmove();

unsigned	rnd();

THING	*find_obj(), *get_item(), *leave_pack(), *new_item(),
	*new_thing(), *wake_monster();

struct room	*roomin();

char	add_line(), *unctrl();
