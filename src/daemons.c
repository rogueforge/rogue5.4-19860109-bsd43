...
sight()
{
	...
	if (!(Proom->r_flags & ISGONE))
	    enter_room(&Hero, chat(Hero.y, Hero.x) == DOOR);
...
stomach()
{
    ...
    {
	oldfood = Food_left;
	Food_left -= ring_eat(LEFT) + ring_eat(RIGHT) + 1;
	if (Amulet)
	    Food_left--;
...
visuals()
{
     ...
     */
    for (tp = Lvl_obj; tp != NULL; tp = next(tp))
	if (!ce(tp->o_pos, Hero) && cansee(tp->o_pos.y, tp->o_pos.x))
